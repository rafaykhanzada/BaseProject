...
namespace BaseProject.Controllers
{
    ...
    public class PlantController : ControllerBase
    {
        ...
        public PlantController(IPlantService plantService, IHttpContextAccessor httpContextAccessor, IPlantRepository plantRepository, IMapper mapper, IAuditLoggerService auditLoggerService)
        {
        ...
        public IActionResult Get(string? Search = null)
        {
            ...
            var result = _plantService.Export(userId, Search);
        ...
        public IActionResult Get(int pageIndex = 0, int pageSize = int.MaxValue, string? Search = null)
        {
            ...
            var jwtToken = Request.Headers["Authorization"].ToString().Replace("Bearer ", "");
            var userId = _auditLoggerService.ExtractJWT(jwtToken);
            return Ok(_plantService.Get(userId, pageIndex,pageSize,Search));
        }
        ...
        public IActionResult Get(int id)
        {
            var jwtToken = Request.Headers["Authorization"].ToString().Replace("Bearer ", "");
            var userId = _auditLoggerService.ExtractJWT(jwtToken);
            return Ok(_plantService.Get(userId, id));
        }
        ...
        public async Task<IActionResult> Post([FromBody] PlantDTO model)
        {
            ...
            if (ModelState.IsValid)
            {
                var jwtToken = Request.Headers["Authorization"].ToString().Replace("Bearer ", "");
                var userId = _auditLoggerService.ExtractJWT(jwtToken);
                return Ok(await _plantService.CreateOrUpdate(userId, model));
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] PlantDTO model)
        {
            if (ModelState.IsValid)
            {
                var jwtToken = Request.Headers["Authorization"].ToString().Replace("Bearer ", "");
        ...
        public IActionResult Delete(int id)
        {
            var jwtToken = Request.Headers["Authorization"].ToString().Replace("Bearer ", "");
            var userId = _auditLoggerService.ExtractJWT(jwtToken);
            return Ok(_plantService.Delete(userId, id));
        }
